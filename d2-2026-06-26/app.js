...
const draftDbName = "reimbursement-draft-db-v2";
const draftStoreName = "drafts";
const invoiceFolderKey = "invoice-folder-handle";
...
const linkInvoiceFolderBtn = $("#linkInvoiceFolderBtn");
...
if (detailSearchInput) detailSearchInput.addEventListener("input", filterDetailRows);
...
function isWechatLongshotItem(item) { return !item?._manualScreenshotPreview && ((item?.source === "微信长截图RapidOCR" && /#\d+/.test(item.fileName || "")) || (item?.source === "微信列表截图" && /#\d+/.test(item.fileName || ""))); }
...
function syncTableControlsToItems() { tableBody.querySelectorAll("[data-id][data-field]").forEach((control) => { const item = items.find((entry) => entry.id === control.dataset.id); if (item) item[control.dataset.field] = control.value; }); }
async function clearDraft() { const draft = await loadDraftPayload().catch(() => null); if (!draft) return setStatus("当前没有已保存的草稿。"); if (!confirm("确认清空本机保存的草稿吗？当前页面数据不会被删除。")) return; await removeDraftPayload(); dirty = false; setStatus("已清空本机草稿。当前页面数据仍保留。"); }
function openDraftDb() { if (!window.indexedDB) return Promise.reject(new Error("IndexedDB unavailable")); if (draftDbPromise) return draftDbPromise; draftDbPromise = new Promise((resolve, reject) => { const request = indexedDB.open(draftDbName, 1); request.onupgradeneeded = () => request.result.createObjectStore(draftStoreName); request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error || new Error("IndexedDB open failed")); }); return draftDbPromise; }
async function draftStore(mode, action) { const db = await openDraftDb(); return new Promise((resolve, reject) => { const tx = db.transaction(draftStoreName, mode); const store = tx.objectStore(draftStoreName); const request = action(store); request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error || new Error("IndexedDB request failed")); tx.onerror = () => reject(tx.error || new Error("IndexedDB transaction failed")); }); }
async function saveDraftPayload(payload) { await draftStore("readwrite", (store) => store.put(payload, draftKey)); try { localStorage.removeItem(draftKey); } catch {} }
async function loadDraftPayload() { let draft = null; try { draft = await draftStore("readonly", (store) => store.get(draftKey)); } catch (error) { console.warn("IndexedDB 草稿读取失败，尝试旧 localStorage。", error); } if (draft) return draft; let raw = ""; try { raw = localStorage.getItem(draftKey) || ""; } catch {} if (!raw) return null; draft = JSON.parse(raw); try { await saveDraftPayload(draft); localStorage.removeItem(draftKey); } catch (error) { console.warn("旧草稿迁移到 IndexedDB 失败。", error); } return draft; }
async function removeDraftPayload() { try { await draftStore("readwrite", (store) => store.delete(draftKey)); } catch (error) { console.warn("IndexedDB 草稿删除失败。", error); } try { localStorage.removeItem(draftKey); } catch {} }
async function saveDraftMeta(key, value) { await draftStore("readwrite", (store) => store.put(value, key)); }
async function loadDraftMeta(key) { try { return await draftStore("readonly", (store) => store.get(key)); } catch { return null; } }
async function removeDraftMeta(key) { try { await draftStore("readwrite", (store) => store.delete(key)); } catch {} }
async function linkInvoiceFolder() { if (!window.showDirectoryPicker) return setStatus("当前浏览器不支持文件夹授权。请使用 Chrome/Edge，或继续手动上传发票 PDF。"); try { const handle = await showDirectoryPicker({ mode: "read" }); await saveDraftMeta(invoiceFolderKey, handle); setStatus("已授权发票文件夹，正在按草稿中的发票名称自动重连 PDF..."); const count = await reconnectInvoicesFromFolder(handle, true); setStatus(count ? `已从授权文件夹重连 ${count} 张发票 PDF，可直接预览和合并导出。` : "已授权文件夹，但未找到与草稿发票名称一致的 PDF。请确认文件名未改动。"); } catch (error) { if (error?.name === "AbortError") return; console.error(error); setStatus("发票文件夹授权失败，请重试或继续手动上传 PDF。"); } }
async function tryReconnectInvoiceFolderOnRestore() { const handle = await loadDraftMeta(invoiceFolderKey); if (!handle || !items.some((item) => invoiceEntries(item).some((entry) => entry.name && !entry.file && !entry.url))) return; const permission = await ensureDirectoryPermission(handle, false); if (permission !== "granted") return setStatus("已恢复草稿。若要自动找回发票 PDF，请点击“重连发票文件夹”并允许访问。"); const count = await reconnectInvoicesFromFolder(handle, false); if (count) setStatus(`已自动从授权文件夹重连 ${count} 张发票 PDF。PDF 可预览并可合并导出。`); }
async function ensureDirectoryPermission(handle, request) { if (!handle?.queryPermission) return "denied"; const opts = { mode: "read" }; let permission = await handle.queryPermission(opts); if (permission !== "granted" && request && handle.requestPermission) permission = await handle.requestPermission(opts); return permission; }
async function reconnectInvoicesFromFolder(handle, requestPermission) { const permission = await ensureDirectoryPermission(handle, requestPermission); if (permission !== "granted") return 0; const needed = collectMissingInvoiceNames(); if (!needed.size) return 0; const files = new Map(); await collectPdfFiles(handle, files, 0); let count = 0; for (const item of items) { const entries = invoiceEntries(item); let changed = false; item.invoiceFiles = entries.map((entry) => { if ((entry.file || entry.url) || !entry.name) return entry; const file = files.get(normalizedInvoiceName(entry.name)); if (!file) return entry; count += 1; changed = true; return { ...entry, file, name: file.name, url: URL.createObjectURL(file) }; }); if (changed) syncInvoiceLegacyFields(item); } if (count) { markDirtyAndSave(); renderAll(); } return count; }
function collectMissingInvoiceNames() { const names = new Set(); for (const item of items) for (const entry of invoiceEntries(item)) if (entry.name && !entry.file && !entry.url) names.add(normalizedInvoiceName(entry.name)); return names; }
...
function filteredDetailItems() { const ordered = sortedItems(); return detailSearchKeyword ? ordered.filter((entry) => detailSearchText(entry, detailSearchKeyword).includes(detailSearchKeyword)) : ordered; }
...
function detailSearchText(item, keyword = "") { const visibleText = normalizeText([item.date, item.category, item.type, item.amount, item.description, item.fileName, item.invoiceFileName, item.invoiceAmount].join(" ")).toLowerCase(); const raw = normalizeText(item.rawText || "").toLowerCase(); return keyword.length >= 4 ? `${visibleText} ${raw}` : visibleText; }
...
async function openInvoicePreview(id, invoiceIndex = 0) {
  ...
  pdfViewerTitle.textContent = invoice.name || "发票 PDF 预览";
  pdfPages.innerHTML = '<div class="pdf-loading">正在生成页面内预览...</div>';
  pdfViewer.classList.add("open");
  document.body.classList.add("viewer-open");

  try {
    const source = invoice.file ? await invoice.file.arrayBuffer() : await (await fetch(invoice.url)).arrayBuffer();
...
function removeInvoiceFile(id) { const item = items.find((entry) => entry.id === id); if (!item) return; revokeInvoiceUrl(item); item.invoiceFile = null; item.invoiceFiles = []; item.invoiceFileName = ""; item.invoiceFileUrl = ""; item.invoiceAmount = ""; if (item.screenshotAmount) item.amount = item.screenshotAmount; markDirtyAndSave(); setStatus(item.screenshotAmount ? `已删除该行全部发票 PDF，金额已恢复为截图识别金额 ${item.screenshotAmount}。` : "已删除该行全部发票 PDF。"); renderAll(); }
...
async function addScreenshotToItem(id, file) { if (!file) return; const item = items.find((entry) => entry.id === id); if (!item) return; item.imageUrl = await imageFileToDraftDataUrl(file); item.screenshotPreviewUrl = item.imageUrl; item.fileName = file.name; item.rawText = ""; item.source = item.source === "微信Excel" ? item.source : "付款截图"; item._manualScreenshotPreview = true; item._wechatPreviewKey = ""; try { const images = await prepareImageForOcr(file); const recognized = await recognizePaymentImage(images, file.name); item.rawText = recognized.text; const parsedPreview = parsePaymentText(recognized.text, file.name); item.screenshotAmount = parsedPreview.amount || item.screenshotAmount; if (!item.amount && parsedPreview.amount) item.amount = parsedPreview.amount; if (!item.date && parsedPreview.date) item.date = parsedPreview.date; if (!item.description && parsedPreview.description) item.description = parsedPreview.description; if ((!item.category || item.category === "其他费用") && parsedPreview.category) item.category = parsedPreview.category; if ((!item.type || item.type === "其他费用") && parsedPreview.type) item.type = parsedPreview.type; item.screenshotPreviewUrl = await imageFileToAmountPreviewDataUrl(file, selectBestAmountPreviewBox(recognized.amountPreviewCandidates, parsedPreview.amount, images.fullWidth, images.fullHeight), images.fullWidth, images.fullHeight) || item.imageUrl; } catch (error) { console.error(error); } markDirtyAndSave(); setStatus(`已上传截图：${file.name}。请确认金额和说明是否正确。`); renderAll(); }
function bindDragEvents(root, selector, idAttr, rowSelector) { let dragId = ""; let overId = ""; let dragCategory = ""; const rowId = (row) => row?.dataset?.rowId || row?.dataset?.previewRowId || ""; const clear = () => root.querySelectorAll(".drag-over,.dragging").forEach((row) => row.classList.remove("drag-over", "dragging")); root.querySelectorAll(selector).forEach((handle) => { handle.addEventListener("pointerdown", (event) => { dragId = handle.getAttribute(idAttr); overId = dragId; const row = handle.closest(rowSelector); dragCategory = row?.dataset?.previewCategory || ""; handle.setPointerCapture?.(event.pointerId); row?.classList.add("dragging"); document.body.classList.add("drag-sorting"); event.preventDefault(); }); handle.addEventListener("pointermove", (event) => { if (!dragId) return; const row = document.elementFromPoint(event.clientX, event.clientY)?.closest?.(rowSelector); const targetId = rowId(row); if (!row || !targetId || targetId === overId) return; if (dragCategory && row.dataset.previewCategory !== dragCategory) return; root.querySelectorAll(".drag-over").forEach((entry) => entry.classList.remove("drag-over")); overId = targetId; row.classList.add("drag-over"); }); handle.addEventListener("pointerup", (event) => { handle.releasePointerCapture?.(event.pointerId); if (dragId && overId && dragId !== overId) reorderItems(dragId, overId); dragId = ""; overId = ""; dragCategory = ""; document.body.classList.remove("drag-sorting"); clear(); }); handle.addEventListener("pointercancel", () => { dragId = ""; overId = ""; dragCategory = ""; document.body.classList.remove("drag-sorting"); clear(); }); }); }
function reorderItems(sourceId, targetId) { const ordered = sortedItems(); const from = ordered.findIndex((item) => item.id === sourceId); const to = ordered.findIndex((item) => item.id === targetId); if (from < 0 || to < 0 || from === to) return; const [moved] = ordered.splice(from, 1); ordered.splice(to, 0, moved); manualSortActive = true; applySortOrder(ordered); markDirtyAndSave(); setStatus("已调整明细顺序，待确认明细、报销单预览和导出会按当前顺序排列。"); renderAll(); }
...
function sortedItems(list = items) { ensureSortOrder(); return [...list].sort((a, b) => manualSortActive ? sortValue(a) - sortValue(b) || dateSortValue(a.date) - dateSortValue(b.date) : dateSortValue(a.date) - dateSortValue(b.date) || sortValue(a) - sortValue(b)); }
function previewOrderedItems() { return sortedItems(); }
function reportOrderedItems() { return previewOrderedItems(); }
function ensureSortOrder() { if (!items.length) return; if (!manualSortActive) { applySortOrder([...items].sort((a, b) => dateSortValue(a.date) - dateSortValue(b.date) || sortValue(a) - sortValue(b))); return; } const missing = items.filter((item) => !Number(item.sortOrder)); if (!missing.length) return; const assigned = items.filter((item) => Number(item.sortOrder)).sort((a, b) => sortValue(a) - sortValue(b) || dateSortValue(a.date) - dateSortValue(b.date)); missing.forEach((item, index) => { item.sortOrder = assigned.length + index + 1; }); }
