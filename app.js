...
async function exportScreenshotsDocx(rows) {
  ...
  downloadBlob(blob, `${exportBaseFileName("报销")}-付款截图.docx`);
  setStatus("已按截图模板导出付款截图 Word。");
}
...
async function exportReportPreviewImage() {
  ...
  try {
    ...
    downloadBlob(blob, `${exportBaseFileName("报销")}-报销单预览.png`);
    setStatus("已导出报销单预览图。");
  } catch (error) {
    console.error(error);
