...
function invoiceCell(item) { return `<div class="invoice-cell ${item.invoiceFileUrl ? "has-invoice" : "no-invoice"}">${item.invoiceFileUrl ? `<button class="invoice-upload invoice-preview-action" type="button" data-invoice-preview="${item.id}">预览</button><button class="invoice-remove" type="button" data-remove-invoice="${item.id}">删发票</button><button class="invoice-file" type="button" data-invoice-preview="${item.id}">${escapeHtml(item.invoiceFileName)}</button>${item.invoiceAmount ? `<span class="invoice-amount">发票金额：${escapeHtml(item.invoiceAmount)}</span>` : ""}` : `<label class="invoice-upload">PDF<input type="file" accept="application/pdf,.pdf" data-invoice-file="${item.id}"></label><span class="invoice-empty">未上传 PDF</span>`}</div>`; }
...
function openInvoicePreview(id) { const item = items.find((entry) => entry.id === id); if (!item?.invoiceFileUrl) return setStatus("PDF 原文件未保留，请重新上传后再预览。"); const opened = window.open(item.invoiceFileUrl, "_blank", "noopener"); if (!opened) setStatus("浏览器拦截了 PDF 预览窗口，请允许弹窗后再点预览，或从导出的发票 PDF 查看。"); }
