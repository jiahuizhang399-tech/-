...
function addScreenshotToItem(id, file) { if (!file) return; const item = items.find((entry) => entry.id === id); if (!item) return; const reader = new FileReader(); reader.onload = () => { item.imageUrl = reader.result; item.fileName = file.name; item.rawText = ""; markDirtyAndSave(); setStatus(`已上传截图：${file.name}。请确认金额和说明是否正确。`); renderAll(); }; reader.readAsDataURL(file); }
