...
function invoiceCell(item) { return `<div class="invoice-cell ${item.invoiceFileUrl ? "has-invoice" : "no-invoice"}"><label class="invoice-upload">PDF<input type="file" accept="application/pdf,.pdf" data-invoice-file="${item.id}"></label>${item.invoiceFileUrl ? `<button class="invoice-remove" type="button" data-remove-invoice="${item.id}">删发票</button><button class="invoice-file" type="button" data-invoice-preview="${item.id}">${escapeHtml(item.invoiceFileName)}</button>${item.invoiceAmount ? `<span class="invoice-amount">发票金额：${escapeHtml(item.invoiceAmount)}</span>` : ""}` : `<span class="invoice-empty">未上传 PDF</span>`}</div>`; }
