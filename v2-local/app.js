...
function saveDraft(showStatus) { try { syncTableControlsToItems(); if (!hasDraftableData()) localStorage.removeItem(draftKey); else localStorage.setItem(draftKey, JSON.stringify(draftPayload())); if (showStatus) setStatus("已保存当前状态到本机浏览器。刷新后会自动恢复。"); } catch (error) { console.error(error); if (showStatus) setStatus("保存失败：浏览器本地存储空间可能不足。可先导出文件备份。"); } }
